...
namespace GUI
{
	void LoadImages()
	{
		if (AS_Logo == NULL)
		{
			...
			Gui.LoadTextureFromMemory(Images::NL_Logo, sizeof Images::NL_Logo, &NL_Logo, &LogoW, &LogoH);
	...
	void NewGui()
	{
		...
		ImGui::Begin("NiggaLose", nullptr, Flags);
		{
			...
			ImGui::BeginChild("Page", MenuConfig::WCS.ChildSize);
			{
				...
				if (MenuConfig::WCS.MenuPage == 2)
				{
					...
					ImGui::SameLine();
					if (ImGui::Combo("###Theme", &MenuConfig::Theme, "NiggarLose"))
						StyleChanger::UpdateSkin(MenuConfig::Theme);
					if (MenuConfig::Theme == 0)
					{	
